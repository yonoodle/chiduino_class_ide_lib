...
boolean Adafruit_ESP8266::connectTCP(String *h, int port) {
...
boolean Adafruit_ESP8266::requestURL(String url, String host_yonoodle) {
  print(F("AT+CIPSEND="));
  println(25 + url.length() + host_yonoodle.length());
...
String Adafruit_ESP8266::return_my_public_IP(String url, String host_yonoodle, int delay_time) 
{

 String my_public_IP = "";
 int count =0;

 connectTCP(F(host_yonoodle), 80) ;

delay(1000);


  while(my_public_IP.equals("") && count<10)
  {
 ...
 //if(find(F("> "))) 
   ...
   while(stream->available()>0)
   {my_public_IP.concat((char)stream->read());}
   

  debug->println(my_public_IP); 
  


  if(my_public_IP.indexOf("html")>=0 && my_public_IP.indexOf("<")>=0)
  { 
   my_public_IP = my_public_IP.substring(my_public_IP.indexOf("html")+6,my_public_IP.indexOf("<"));

      debug->print("(IP=");
   debug->print(my_public_IP);
   debug->println(")");
    return(my_public_IP);
   }
 ...
 debug->println("fail to get public IP");


  
  //return "fail to get public ip";
}
