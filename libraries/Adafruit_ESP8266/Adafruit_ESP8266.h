...
class Adafruit_ESP8266 : public Print {
 public:
  ...
  boolean   hardReset(void),
           ...
           // connectTCP(String *h, int port),            
            requestURL(String url,String host_yonoodle),
            requestURL(Fstr *url),
            requestURL(char* url);

  String return_my_public_IP(String url, String host_yonoodle, int delay_time);
  ...
  void      closeAP(void),
            ...
            clear_buffer(void),
            clear_buffer_before_prompt_sign(void),
