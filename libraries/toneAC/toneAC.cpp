...
#else
void toneAC_tiny(unsigned long frequency, unsigned long length) {
...
void noToneACHIGH() {
  ...
  PWMT1PORT |= _BV(PWMT1AMASK); // Set timer 1 PWM pins to HIGH. (some speaker module will have current drain when grounded and get heated... to prevent this)
  PWMT1PORT |= _BV(PWMT1BMASK); // Other timer 1 PWM pin also to HIGH.
